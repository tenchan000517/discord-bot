...
class DynamoDBChecker:
    ...
    def get_server_points(self, server_id):
        """特定のサーバーのポイントデータを取得"""
        table = self.dynamodb.Table('discord_users')
        try:
            # サーバーIDでフィルタリングするスキャンを実行
            response = table.scan(
...
def print_formatted(title, data):
    ...
    if "のポイントデータ" in title:
        ...
        for item in data:
            ...
            print(f"表示名: {item.get('displayName', 'N/A')}")
            print("-" * 50)
    elif "のポイント履歴" in title:
        # ポイント履歴の表示
        for item in data:
            print("\n消費履歴レコード:")
            print(f"サーバーID: {item.get('server_id', 'N/A')}")
            ...
            print(f"スレッドID: {item.get('thread_id', 'N/A')}")
            print(f"ポイント: {item.get('points', 'N/A')}")
