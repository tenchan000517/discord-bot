...
class RewardManager:
        ...
        try:
            ...
            try:
                if reward_type == 'COUPON':
                    claim_code = await self.coupon_service.generate_coupon(points)
                elif reward_type == 'NFT':
                    claim_code = await self.nft_service.mint_nft(user_id, {})
                elif reward_type == 'TOKEN':
                    claim_code = await self.token_service.transfer_tokens(user_id, points)
                else:
    ...
    async def claim_reward(self, user_id: str, server_id: str, reward_type: str, points: int) -> Optional[Reward]:
        """報酬を請求"""
        try:
            # サーバー設定を取得
