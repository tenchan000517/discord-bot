...
class PointsConsumption(commands.Cog):
    ...
    #         elif interaction.data['custom_id'].startswith('approve_'):
    ...
    #         elif interaction.data['custom_id'] == 'show_consumption_modal':
    #             # ユーザーの利用可能ポイントを取得
    ...
    #                 settings, 
    #                 # settings.point_consumption_settings.modal_settings,
    #                 available_points
    #             )
            ...
            if success:
                if consumption_settings.completion_message_enabled:
                    await interaction.channel.send(
                        f"{interaction.user.mention}のポイント消費申請がキャンセルされました。"
                    )

                # ログ記録
                if consumption_settings.logging_enabled and 'cancel' in consumption_settings.logging_actions:
                    log_channel = self.bot.get_channel(int(consumption_settings.logging_channel_id))
                    if log_channel:
                        await log_channel.send(
                            f"管理者 {interaction.user.mention} がポイント消費申請をキャンセルしました。"
                        )
