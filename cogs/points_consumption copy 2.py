...
class PointsConsumption(commands.Cog):
    ...
    async def on_interaction(self, interaction: discord.Interaction):
        ...
        try:
            ...
            if not interaction.data or 'custom_id' not in interaction.data:
                print("[DEBUG] Invalid interaction received, no 'custom_id' present.")
                return

            custom_id = interaction.data['custom_id']
            print(f"[DEBUG] Received interaction with custom_id: {custom_id}")

            # サーバー設定の取得
            settings = await self.bot.get_server_settings(str(interaction.guild_id))
            print(f"[DEBUG] Retrieved settings: {settings}")

            # modal_settings を参照する直前のログ
            if settings and 'feature_settings' in settings and 'point_consumption' in settings['feature_settings']:
                modal_settings = settings['feature_settings']['point_consumption']['modal_settings']
                print(f"[DEBUG] modal_settings.fields.wallet: {modal_settings['fields']['wallet']}")
            ...
            # point_consumption_settings 全体を確認
            print(f"[DEBUG] point_consumption_settings: {settings.point_consumption_settings.__dict__}")

            # modal_settings が存在するか確認
            if hasattr(settings.point_consumption_settings, "modal_settings"):
                print(f"[DEBUG] modal_settings: {settings.point_consumption_settings.modal_settings.__dict__}")

                # fields が存在するか確認
                if hasattr(settings.point_consumption_settings.modal_settings, "fields"):
                    print(f"[DEBUG] modal_settings.fields: {settings.point_consumption_settings.modal_settings.fields}")
                    # wallet フィールドの値を確認
                    wallet_field = settings.point_consumption_settings.modal_settings.fields.get("wallet", None)
                    print(f"[DEBUG] wallet field value: {wallet_field}")
                else:
                    print("[DEBUG] modal_settings.fields not found.")
            else:
                print("[DEBUG] modal_settings not found in point_consumption_settings.")
            ...
            if custom_id.startswith('consume_points_'):
                ...
                if not interaction.response.is_done():
                    await interaction.response.defer(ephemeral=True)  # 応答保留
                ...
                modal = PointConsumptionModal(settings, available_points, unit_id)
                if not interaction.response.is_done():
                ...
                print(f"[DEBUG] Unknown custom_id: {custom_id}")
                if not interaction.response.is_done():
                    await interaction.response.send_message(
                        "無効な操作が検出されました。",
                        ephemeral=True
            ...
            print(f"[ERROR] HTTPException in on_interaction: {e}")
            if not interaction.response.is_done():
                await interaction.response.send_message(
            ...
            print(f"[ERROR] General error in on_interaction: {e}")
            print(traceback.format_exc())
            if not interaction.response.is_done():
                await interaction.response.send_message(
                    "予期しないエラーが発生しました。",
    ...
    async def handle_consume_button(self, interaction: discord.Interaction):
        ...
        try:
            ...
            if not settings or not settings.point_consumption_settings:
                await interaction.followup.send("ポイント消費機能の設定が見つかりません。", ephemeral=True)
                return
            ...
            if not point_unit:
                await interaction.followup.send("無効なポイントプールです。", ephemeral=True)
                return
            ...
            try:
                ...
                await interaction.followup.send("スレッドの作成に失敗しました。", ephemeral=True)
                return
            ...
            try:
                ...
                await interaction.followup.send("スレッドの設定に失敗しました。", ephemeral=True)
                return
