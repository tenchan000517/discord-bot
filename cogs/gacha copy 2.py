...
class GachaView(discord.ui.View):
    ...
    async def gacha_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            current_data = await self.bot.point_manager.get_points(server_id, user_id)
            current_points_data = current_data.get('points', {}) if isinstance(current_data, dict) else {}
            print(f"[DEBUG] 現在のポイント: {current_data}, 新しい合計ポイント: {current_points_data}")
    ...
    async def check_points(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            user_rank = next(
                (i + 1 for i, rank in enumerate(all_rankings) if rank['user_id'] == user_id),
                len(all_rankings) + 1
            )
