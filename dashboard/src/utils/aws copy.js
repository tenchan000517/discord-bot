...
export class AWSWrapper {
    ...
    #normalizePoints(points) {
        if (typeof points === 'number' || points instanceof Decimal) {
    ...
    #normalizeUserData(userData) {
        return {
            ...
            points: this.#normalizePoints(userData.points),
            lastActive: userData.last_active || null,
            lastGachaDate: userData.last_gacha_date || null,
    ...
    async getServerUsers(serverId) {
        try {
            ...
            const response = await this.docClient.send(command);
            return response.Items.map(user => this.#normalizeUserData(user));
        } catch (error) {
    ...
    async updateUserPoints(serverId, userId, points) {
        try {
            ...
            const response = await this.docClient.send(command);
            return this.#normalizeUserData(response.Attributes);
        } catch (error) {
    ...
    //             ...user,
    //             points: normalizePoints(user.points)
    //         }));
    ...
    //         // 現在のポイントが数値型か確認
    //         const currentPoints = typeof existingData.Item?.points === 'number' ? existingData.Item.points : 0;
